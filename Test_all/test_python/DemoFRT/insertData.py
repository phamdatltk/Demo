...
# Add a document to the index fde_demo_index_two_replica_two_shard.
for i in range(1000):
    document = {
        'title': random.choice(listCity),
        'checkYear': random.randint(1900, 2025),
        'description': "Data of fde_demo_index_two_replica_two_shard",
        'timestamp': datetime.utcnow().isoformat()  # Current timestamp
    }

    response = client.index(index='fde_demo_index_two_replica_two_shard', body=document)
    print(response)
