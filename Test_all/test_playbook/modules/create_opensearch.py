...
from ansible.module_utils.create_opensearch import create_vm
import time
...
from ansible.module_utils.util import list_vm_opensearch
import http.client
...
def create_vm_argument_spec():
    return dict(
        ...
        data_node=dict(type=int, required=True),
...
class Create_VM(VcdAnsibleModule):
    ...
    def create_vm(self):
        ...
        data_node = params.get('data_node')
        print(type(data_node))
        print("print data_node:", data_node)
        vms_list = list_vm_opensearch(cluster_name, cluster_id, data_node)
        print("print vms_list:", vms_list)
        ...
        for i in range(400):
            for vm in vms_list:
                ...
                while True:
                    vm_status_check_Unresolved = get_vm_status(self.client, vapp_uuid, vm)
                    if vm_status_check_Unresolved == 'Unresolved':
                        time.sleep(5)
                    else:
                        break
                non_busy_vm = check_busy_vcd(self.client, vapp_uuid, vm)
                non_busy_vms.append(non_busy_vm)
                print("print non_busy_vms:", non_busy_vms)
