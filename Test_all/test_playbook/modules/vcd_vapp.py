...
EXAMPLES = '''
- name: add VM org direct network to VM vapp
  vcd_vapp:
    isEnableVDCGroup: true
    user: "tpbank-local"
    password: "LU9R7yefV6Dgw,s`+i#U"
    org: "TPBANK-ORG"
    vapp_name: "phongvd8-test"
    vdc: "TPBANK-VDC"
    network: "phongvd8"
    operation: add_org_network
  register: output
  tags: add-network-vapp
'''
...
from ansible.module_utils.connect_orgVdcNetwork_to_Vapp import connect_orgVdcNetwork_to_Vapp
from ansible.module_utils.get_vapp_network import check_vapp_network
...
class Vapp(VcdAnsibleModule):
    ...
    def add_org_network(self):
        ...
        if isEnableVDCGroup:
            vapp_id = get_vapp_id(self.client, vapp_name, vdc_name)
            vapp_uuid = vapp_id.replace("vapp-", '')
            if check_vapp_network(self.client, network, vapp_uuid):
                response['warnings'] = "Org Vdc Network {} is connected to Vapp.".format(network)
            else:
                orgVdcNetwork = get_orgVdcNetwork(self.client, network)
                connect_orgVdcNetwork_to_Vapp(self.client, vapp_id, orgVdcNetwork)
                msg = 'Org Vdc Network {} has been connected to Vapp'
                response['msg'] = msg.format(network)
                response['changed'] = True
        else:
            try:
                vapp = self.get_vapp()
