...
options:
    ...
    vm_type:
        description:
            - type of VM (master, worker, keepalive, nfs)
        required: true
    vm_count:
        description:
            - count of VM
...
- name: create VM parrallel
  create_vm:
    ...
    vapp_template_keepalive_name: "fke-keepalive-v1-29112021"
...
from ansible.module_utils.create_master_slave_keepalive import create_vm
import time
...
class Create_VM(VcdAnsibleModule):
    ...
    def create_vm(self):
        ...
        if num_vm_id_diff_empty == len(vms_id):
            ...
            create_vm(self.client, vdc, catalogName, vapp_template_master_name, vapp_template_worker_name, vapp_template_keepalive_name, vapp_name, vm_password, vms_list, network_name)
            time.sleep(10)
            vms_status = list()
            non_busy_vms = list()
            num_VM_powerOff = 0
    ...
    def scale_vm(self):
        ...
        if num_vm_id_diff_empty == len(vms_id):
            ...
            create_vm_without_master(self.client, vdc, catalogName, vapp_template_worker_name, vapp_template_keepalive_name, vapp_name, vm_password, vms_list, network_name)
            time.sleep(10)
            vms_status = list()
            num_VM_powerOff = 0
            non_busy_vms = list()
